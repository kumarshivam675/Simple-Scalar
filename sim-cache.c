...
il1_access_fn(enum mem_cmd cmd,         /* access cmd, Read or Write */
    ...
    if (stream_buffer){
            if (prefetch_access(stream_buffer, baddr, bsize, now)){ /* one cycle delay to get data in l1 */
                cache_il1->misses--;
                cache_il1->hits++;
		//printf("check_12");
		cache_il1->pc_hits++;
	...
	cache_il1->pc_misses++;// miss handler itself in prefetech access
	//printf("check_13");
    }
...
"      -cache:dl1 ul1:256:32:1:l -cache:dl2 ul2:1024:64:2:l\n"
  ...
  opt_reg_flag(odb, "-cache:icompress",
               ...
               /* print */TRUE, NULL);
  opt_reg_flag(odb, "-pc", "Use a stream buffer",
  ...
  if (!mystricmp(cache_il1_opt, "none"))
  ...
  else /* il1 is defined */
    {
      ...
      cache_il1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
        ...
        /* use an prefetch cache? */
        if (!stream_buffer_opt){
            stream_buffer = NULL;
        } else {
            stream_buffer = cache_create("pf", 1, bsize, /* balloc */FALSE,
