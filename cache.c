                ...
                struct stat_sdb_t *sdb) /* stats database */
{
  ...
  sprintf(buf, "%s.pc_misses", name);
  stat_reg_counter(sdb, buf, "total number of stream buffer misses", &cp->pc_misses, 0, NULL);
  sprintf(buf, "%s.pc_hits", name);
  stat_reg_counter(sdb, buf, "total number of stream buffer hits", &cp->pc_hits, 0, NULL);
...
prefetch_access(struct cache_t *cp,       /* cache to access */
    ...
    if (cache_probe(cp, addr)){
        ...
        /* **HIT** */
        cp->hits++;
        /* hit substitute l1 in head (FIFO) (4 blocks are used) replace found blocks*/
	
	for (i = 0; i < found; i++){
            cache_access(cp, 0/*read*/, addr + (nbytes) * (i + 1), NULL, nbytes, now, NULL, NULL);
        }
        /* return first cycle data */
        return 1;
    }

    /* **MISS** flush cache and access next (addr + blk size * 4) */
    cp->misses++;

    cache_flush(cp, now);
    
    // access (addr + 1) (now + i because L1 is busy)

    for (i = 0; i < 4; i++){ // filling new stream
        cache_access(cp, 1/*write*/, addr + (nbytes) * (i + 1), NULL, nbytes, now + i, NULL, NULL);
    }
    return 0;
}
