...
struct cache_t
{
  ...
  counter_t pc_misses; //total number of misses in stream buffer
  counter_t pc_hits;    //total number of hits in stream buffer
...
unsigned intcache_add(struct cache_t *cp,enum mem_cmd cmd,md_addr_t addr,
void *vp, int nbytes, tick_t now, byte_t **udata, md_addr_t *repl_addr);  
